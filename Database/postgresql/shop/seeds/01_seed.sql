...
SELECT setval(pg_get_serial_sequence('product_options', 'id'), COALESCE((SELECT MAX(id) FROM product_options), 1), true);
...
SELECT setval(pg_get_serial_sequence('order_addresses', 'id'), COALESCE((SELECT MAX(id) FROM order_addresses), 1), true);
