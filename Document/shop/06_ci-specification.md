...
`shop`용 모든 구현체는 프로젝트별 개별 workflow 파일을 가진다.
...
- `shop-java-springboot-maven-jpa-postgresql-ci.yml`
- `shop-java-springboot-gradle-jdbc-mysql-ci.yml`
- `shop-typescript-nestjs-npm-typeorm-postgresql-ci.yml`
- `shop-typescript-nestjs-npm-knex-mysql-ci.yml`

경로 감시 규칙:

- 각 workflow는 자기 자신의 workflow 파일과 해당 프로젝트 폴더만 감시해야 한다.
- 프로젝트 폴더 경로는 `Backend/shop/<project-folder>` 규칙을 따른다.
...
`shop` 도메인 CI는 최소한 아래 회귀를 커버해야 한다.
...
`shop` 구현체의 CI가 준비됐다고 보려면 아래를 만족해야 한다.
