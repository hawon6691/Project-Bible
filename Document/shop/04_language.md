...
이 문서는 `shop` 서비스에서 실제로 생성해야 하는 백엔드 구현 프로젝트 수와 폴더명 규칙, CLI 식별 규칙을 정의하는 기준 문서다.
기존의 일반 기술 비교 문서가 아니라, 현재 저장소 구조에 맞는 실제 생성 기준을 고정하는 것이 목적이다.

`post`와 `shop`은 동일한 기술 매트릭스를 사용하며, 이 문서는 그중 `shop` 도메인에 대한 기준만 다룬다.

## 서비스 기준 생성 프로젝트 수

`shop` 서비스는 아래 기술 축을 기준으로 백엔드 프로젝트를 생성한다.
...
`shop` 도메인 구현체는 아래 경로 아래에 배치한다.

`Backend/shop/<project-folder>`
...
공용 Python CLI는 `shop` 전용이 아니라 저장소 전체를 제어하는 공용 도구로 둔다.
...
- `shop` 서비스 실행 대상은 `shop-...` 접두어를 가진 프로젝트명으로 지정한다.
