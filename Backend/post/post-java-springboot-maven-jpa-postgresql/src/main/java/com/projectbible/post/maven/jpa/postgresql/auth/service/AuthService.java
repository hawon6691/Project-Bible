package com.projectbible.post.maven.jpa.postgresql.auth.service;
...
import com.projectbible.post.maven.jpa.postgresql.auth.entity.UserRefreshTokenEntity;
import com.projectbible.post.maven.jpa.postgresql.auth.repository.AuthRepository;
...
import com.projectbible.post.maven.jpa.postgresql.common.security.TokenService;
...
public class AuthService {
    ...
    public UserSummaryDto signup(SignupRequestDto request) {
        ...
        UserEntity user = new UserEntity(email, encoder.encode(request.password()), request.nickname().trim(), "ACTIVE");
    ...
    public LoginResponseDto login(LoginRequestDto request) {
        ...
        return issueUserTokens(actor, user);
    }
    ...
    public LoginResponseDto refresh(RefreshRequestDto request) {
        ...
        return issueUserTokens(new CurrentActor(user.getId(), user.getEmail(), "USER", "user"), user);
    }

    public com.projectbible.post.maven.jpa.postgresql.common.api.MessageResponse logout(CurrentActor actor) {
        if (!actor.isUser()) {
            throw new AppException("FORBIDDEN", "User access required", HttpStatus.FORBIDDEN);
        }
        authRepository.revokeAllUserTokens(actor.id());
        return new com.projectbible.post.maven.jpa.postgresql.common.api.MessageResponse("Logged out successfully");
    }

    private LoginResponseDto issueUserTokens(CurrentActor actor, UserEntity user) {
        ...
        return new LoginResponseDto(accessToken, refreshToken, 1800, toUserSummary(user));
    }
