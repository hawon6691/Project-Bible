package com.projectbible.post.post.service;
...
public class PostQueryService {
    ...
    public com.projectbible.post.admin.dto.AdminDtos.StatusResponseDto adminSetStatus(long postId, String status) {
        PostEntity post = postRepository.findDetail(postId)
            .orElseThrow(() -> new AppException("POST_NOT_FOUND", "Post not found", HttpStatus.NOT_FOUND));
        String upper = normalizeStatus(status);
        post.setStatus(upper);
        postRepository.save(post);
        return new com.projectbible.post.admin.dto.AdminDtos.StatusResponseDto(post.getId(), post.getStatus(), post.getUpdatedAt());
    }
