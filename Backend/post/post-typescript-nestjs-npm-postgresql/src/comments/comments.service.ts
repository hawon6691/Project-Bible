...
export class CommentsService {
  ...
  async list(postId: number, query: Record<string, string | undefined>) {
    ...
    const countRows = await this.dataSource.query(
      `select count(*)::int as count from comments c where c.post_id = $1 and c.status <> 'DELETED'`,
      [postId],
    );
  ...
  async create(user: CurrentUserPayload, postId: number, body: Record<string, unknown>) {
    ...
    const inserted = await this.dataSource.query(
      `insert into comments (post_id, user_id, content, status) values ($1, $2, $3, 'ACTIVE') returning id`,
      [postId, user.sub, content],
    );
    await this.dataSource.query(`update posts set comment_count = comment_count + 1, updated_at = now() where id = $1`, [postId]);
    return this.one(Number((inserted[0] as { id: number }).id));
  }
  ...
  async update(user: CurrentUserPayload, commentId: number, body: Record<string, unknown>) {
    ...
    const content = ensureString(body.content, "content");
    await this.dataSource.query(`update comments set content = $2, updated_at = now() where id = $1`, [commentId, content]);
  ...
  async remove(user: CurrentUserPayload, commentId: number) {
    ...
    await this.dataSource.query(`update comments set status = 'DELETED', deleted_at = now(), updated_at = now() where id = $1`, [commentId]);
    await this.dataSource.query(`update posts set comment_count = greatest(comment_count - 1, 0), updated_at = now() where id = $1`, [Number(current.postId)]);
  ...
  async adminList(query: Record<string, string | undefined>) {
    ...
    if (query.postId) {
      ...
      filters.push(`c.post_id = $${values.length}`);
    }
    if (query.search) {
      values.push(`%${query.search.trim()}%`);
      filters.push(`(c.content ilike $${values.length} or u.nickname ilike $${values.length})`);
    }
    if (query.status) {
    ...
    const countRows = await this.dataSource.query(`select count(*)::int as count from comments c join users u on u.id = c.user_id ${where}`, values);
    const totalCount = Number((countRows[0] as { count: number }).count ?? 0);
  ...
  async adminSetStatus(commentId: number, status: string) {
    ...
    await this.dataSource.query(
      `update comments
       set status = $2, deleted_at = case when $2 = 'DELETED' then now() else deleted_at end, updated_at = now()
       where id = $1`,
      [commentId, upperStatus],
    );
