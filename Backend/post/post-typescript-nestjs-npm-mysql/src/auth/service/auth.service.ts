...
export class AuthService {
  ...
  async login(body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const password = ensureString(body.password, "password");
    const rows = await this.dataSource.query(
      `select id, email, password_hash as passwordHash, nickname, status, created_at as createdAt
  ...
  async refresh(body: Record<string, unknown>): Promise<AuthTokens> {
    ...
    const rows = await this.dataSource.query(
      `select urt.id, urt.user_id as userId, u.email
       ...
       where urt.token_key = ? and urt.revoked = false and urt.expires_at > now() and u.deleted_at is null`,
      [tokenKey],
    );
    ...
    await this.dataSource.query(`update user_refresh_tokens set revoked = true, updated_at = now() where id = ?`, [tokenRow.id]);
  ...
  async logout(user: CurrentUserPayload): Promise<Record<string, string>> {
    ...
    await this.dataSource.query(
      `update user_refresh_tokens set revoked = true, updated_at = now() where user_id = ? and revoked = false`,
      [user.sub],
    );
