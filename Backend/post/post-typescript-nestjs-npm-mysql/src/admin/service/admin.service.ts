...
export class AdminService {
  ...
  async logout(user: CurrentUserPayload): Promise<Record<string, string>> {
    ...
    await this.dataSource.query(
      `update admin_refresh_tokens set revoked = true, updated_at = now() where admin_id = ? and revoked = false`,
      [user.sub],
    );
  ...
  async me(user: CurrentUserPayload): Promise<Record<string, unknown>> {
    this.assertAdmin(user);
    const rows = await this.dataSource.query(
      `select id, email, name, status, created_at as createdAt from admins where id = ?`,
      [user.sub],
    );
    const admin = rows[0] as Record<string, unknown> | undefined;
    if (!admin) {
  ...
  async dashboard(): Promise<Record<string, unknown>> {
    const queries = await Promise.all([
      this.scalar(`select count(*) as count from boards where status <> 'DELETED'`),
      this.scalar(`select count(*) as count from posts where status <> 'DELETED'`),
      this.scalar(`select count(*) as count from comments where status <> 'DELETED'`),
      this.scalar(`select count(*) as count from posts where status = 'HIDDEN'`),
      this.scalar(`select count(*) as count from comments where status = 'HIDDEN'`),
    ]);
