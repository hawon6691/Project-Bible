...
export class OrdersService {
  ...
  async create(user: CurrentUserPayload, body: Record<string, unknown>) {
    ...
    const items = await this.dataSource.query(
      `select c.id, c.product_id as productId, c.product_option_id as productOptionId, c.quantity,
       ...
       where c.user_id = ? and c.id in (${cartPlaceholders})`,
    ...
    for (const item of items as Array<Record<string, unknown>>) {
      ...
      if (item.productOptionId) {
        await this.dataSource.query(`update product_options set stock = stock - ?, updated_at = now() where id = ?`, [item.quantity, item.productOptionId]);
      }
      await this.dataSource.query(`update products set stock = stock - ?, updated_at = now() where id = ?`, [item.quantity, item.productId]);
    }

    await this.dataSource.query(`delete from cart_items where user_id = ? and id in (${cartPlaceholders})`, [user.sub, ...cartItemIds]);
  ...
  async list(user: CurrentUserPayload, query: Record<string, string | undefined>) {
    ...
    const filters: string[] = [`o.user_id = ?`];
    if (query.status) {
      values.push(query.status.toUpperCase());
      filters.push(`o.order_status = ?`);
    }
    const where = `where ${filters.join(" and ")}`;
    const countRows = await this.dataSource.query(`select count(*) as count from orders o ${where}`, values);
  ...
  async one(orderId: number, user?: CurrentUserPayload) {
    ...
    const [orderAddressRows, orderItemsRows, paymentRows] = await Promise.all([
      this.dataSource.query(`select recipient_name as recipientName, phone, zip_code as zipCode, address1, address2 from order_addresses where order_id = ?`, [orderId]),
  ...
  async cancel(user: CurrentUserPayload, orderId: number) {
    ...
    await this.dataSource.query(`update orders set order_status = 'CANCELLED', cancelled_at = now(), updated_at = now() where id = ?`, [orderId]);
  ...
  async adminList(query: Record<string, string | undefined>) {
    ...
    const where = `where ${filters.join(" and ")}`;
    const countRows = await this.dataSource.query(`select count(*) as count from orders o join users u on u.id = o.user_id ${where}`, values);
    const totalCount = Number((countRows[0] as { count: number }).count ?? 0);
    values.push(limit, offset);
    const rows = await this.dataSource.query(
      `select o.id, o.order_number as "orderNumber", o.order_status as "orderStatus", o.payment_status as "paymentStatus",
    ...
    return {
      ...
      meta: pageMeta(page, limit, totalCount),
    };
  }
  ...
  async adminSetStatus(orderId: number, orderStatus: string) {
    ...
    const updated = await this.one(orderId);
    return { id: updated.order.id, orderStatus: updated.order.orderStatus, updatedAt: updated.order.updatedAt };
  }
