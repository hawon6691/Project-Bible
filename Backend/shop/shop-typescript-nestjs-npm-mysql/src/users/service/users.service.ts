...
export class UsersService {
  ...
  async update(user: CurrentUserPayload, body: Record<string, unknown>) {
    ...
    await this.dataSource.query(
      `update users
       set name = ?, phone = ?, password_hash = coalesce(?, password_hash), updated_at = now()
       where id = ?`,
      [name, phone, passwordHash, user.sub],
    );
    const updatedRows = await this.dataSource.query(
      `select id, email, name, phone, status, updated_at as updatedAt
       from users where id = ?`,
      [user.sub],
    );
  ...
  async remove(user: CurrentUserPayload) {
    this.assertUser(user);
    await this.dataSource.query(`update users set status = 'DELETED', deleted_at = now(), updated_at = now() where id = ?`, [user.sub]);
    await this.dataSource.query(`update user_refresh_tokens set revoked = true, updated_at = now() where user_id = ? and revoked = false`, [user.sub]);
