...
export class CartService {
  ...
  async update(user: CurrentUserPayload, cartItemId: number, body: Record<string, unknown>) {
    ...
    await this.dataSource.query(
      ...
      [quantity, cartItemId, user.sub],
    );
    return this.one(user, cartItemId);
  }

  async remove(user: CurrentUserPayload, cartItemId: number) {
    this.assertUser(user);
    await this.one(user, cartItemId);
    await this.dataSource.query(`delete from cart_items where id = ? and user_id = ?`, [cartItemId, user.sub]);
    return { message: "Cart item deleted successfully" };
  }

  async clear(user: CurrentUserPayload) {
    this.assertUser(user);
    await this.dataSource.query(`delete from cart_items where user_id = ?`, [user.sub]);
    return { message: "Cart cleared successfully" };
  }

  async one(user: CurrentUserPayload, cartItemId: number) {
    const rows = await this.dataSource.query(
      `select id, product_id as productId, product_option_id as productOptionId, quantity from cart_items where id = ? and user_id = ?`,
      [cartItemId, user.sub],
    );
  ...
  private async ensureStock(productId: number, optionId: number | null, quantity: number): Promise<void> {
    const productRows = await this.dataSource.query(`select id, stock, status from products where id = ? and status = 'ACTIVE'`, [productId]);
    ...
    if (optionId !== null) {
      const optionRows = await this.dataSource.query(`select stock from product_options where id = ? and product_id = ?`, [optionId, productId]);
