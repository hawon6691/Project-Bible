...
export class AdminService {
  ...
  async logout(user: CurrentUserPayload) {
    ...
    await this.dataSource.query(`update admin_refresh_tokens set revoked = true, updated_at = now() where admin_id = $1 and revoked = false`, [user.sub]);
    return { message: "Admin logged out successfully" };
  }

  async me(user: CurrentUserPayload) {
    this.assertAdmin(user);
    const rows = await this.dataSource.query(`select id, email, name, status, created_at as "createdAt" from admins where id = $1`, [user.sub]);
    const admin = rows[0] as Record<string, unknown> | undefined;
    if (!admin) {
  ...
  async dashboard() {
    const [pending, paid, shipping, cancelled, activeProducts, hiddenProducts, deletedProducts, activeUsers, todayUsers, activeReviews, hiddenReviews, deletedReviews] = await Promise.all([
      this.scalar(`select count(*)::int from orders where order_status = 'PENDING'`),
      this.scalar(`select count(*)::int from orders where order_status = 'PAID'`),
      this.scalar(`select count(*)::int from orders where order_status = 'SHIPPING'`),
      this.scalar(`select count(*)::int from orders where order_status = 'CANCELLED'`),
      this.scalar(`select count(*)::int from products where status = 'ACTIVE'`),
      this.scalar(`select count(*)::int from products where status = 'HIDDEN'`),
      this.scalar(`select count(*)::int from products where status = 'DELETED'`),
      this.scalar(`select count(*)::int from users where status = 'ACTIVE' and deleted_at is null`),
      this.scalar(`select count(*)::int from users where created_at::date = current_date and deleted_at is null`),
      this.scalar(`select count(*)::int from reviews where status = 'ACTIVE' and deleted_at is null`),
      this.scalar(`select count(*)::int from reviews where status = 'HIDDEN' and deleted_at is null`),
      this.scalar(`select count(*)::int from reviews where status = 'DELETED'`),
    ]);
  ...
  private async issueAdminTokens(payload: CurrentUserPayload) {
    ...
    await this.dataSource.query(`insert into admin_refresh_tokens (admin_id, token_key, expires_at, revoked) values ($1, $2, now() + interval '7 days', false)`, [payload.sub, this.hashToken(refreshToken)]);
