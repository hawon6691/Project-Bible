...
export class ProductsService {
  ...
  async list(query: Record<string, string | undefined>) {
    ...
    const countRows = await this.dataSource.query(`select count(*)::int as count from products p ${where}`, values);
  ...
  async one(productId: number) {
    const rows = await this.dataSource.query(
      `select p.id, p.category_id as "categoryId", p.name, p.description, p.price, p.stock, p.status,
       ...
       from products p where p.id = $1 and p.status <> 'DELETED'`,
      [productId],
    );
    ...
    const [options, images] = await Promise.all([
      this.dataSource.query(
        `select id, product_id as "productId", name, value, additional_price as "additionalPrice", stock,
         ...
         from product_options where product_id = $1 order by id asc`,
        [productId],
      ),
      this.dataSource.query(
        `select id, product_id as "productId", image_url as "imageUrl", is_primary as "isPrimary", display_order as "displayOrder",
                created_at as "createdAt", updated_at as "updatedAt"
         from product_images where product_id = $1 order by display_order asc, id asc`,
        [productId],
      ),
    ]);
  ...
  async create(body: Record<string, unknown>) {
    ...
    return this.one(Number((rows[0] as { id: number }).id));
  }
  ...
  async remove(productId: number) {
    ...
    await this.dataSource.query(`update products set status = 'DELETED', deleted_at = now(), updated_at = now() where id = $1`, [productId]);
  ...
  async updateOption(optionId: number, body: Record<string, unknown>) {
    const rows = await this.dataSource.query(
      `select id, product_id as "productId", name, value, additional_price as "additionalPrice", stock from product_options where id = $1`,
      [optionId],
    );
    ...
    const updated = await this.dataSource.query(
      `update product_options
       set name = $2, value = $3, additional_price = $4, stock = $5, updated_at = now()
       where id = $1
       returning id, product_id as "productId", name, value, additional_price as "additionalPrice", stock,
                 created_at as "createdAt", updated_at as "updatedAt"`,
      [
  ...
  async removeOption(optionId: number) {
    const rows = await this.dataSource.query(`delete from product_options where id = $1 returning id`, [optionId]);
    if (rows.length === 0) {
      throw new AppException("PRODUCT_OPTION_NOT_FOUND", "Product option not found", HttpStatus.NOT_FOUND);
    }
  ...
  async updateImage(imageId: number, body: Record<string, unknown>) {
    ...
    const updated = await this.dataSource.query(
      `update product_images
       set image_url = $2, is_primary = $3, display_order = $4, updated_at = now()
       where id = $1
       returning id, product_id as "productId", image_url as "imageUrl", is_primary as "isPrimary", display_order as "displayOrder",
                 created_at as "createdAt", updated_at as "updatedAt"`,
      [
  ...
  async removeImage(imageId: number) {
    const rows = await this.dataSource.query(`delete from product_images where id = $1 returning id`, [imageId]);
    if (rows.length === 0) {
      throw new AppException("PRODUCT_NOT_FOUND", "Product image not found", HttpStatus.NOT_FOUND);
    }
  ...
  private async ensureCategory(categoryId: number): Promise<void> {
    const rows = await this.dataSource.query(`select id from categories where id = $1 and status <> 'DELETED'`, [categoryId]);
    if (rows.length === 0) {
